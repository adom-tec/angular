	...
	ngOnInit() {
		...
		this.professionalService.getProfessionals().subscribe(
			(data) => {
				...
				this.professionals = [ pro ].concat(
					data.map((pro) => {
						return {
							Id: pro.ProfessionalId,
							Name: `${pro.user.FirstName} ${pro.user.SecondName || ''} ${pro.user.Surname} ${pro.user
								.SecondSurname || ''}`
						};
					})
				);
  ...
  */
   ...
   */
	public downloadReport() {
		...
		this.http
			.get(`${environment.apiUrl}/api/reports/${this.filters.ReportType}`, {
				params: params,
			...
			.subscribe(
				(res) => {
					FileSaver.saveAs(
						...
						`Reporte copagos - ${this.filters.ReportType === 'copayment'
							? 'Consolidado'
							: 'Nómina'} - ${moment(Date.now()).format('DD-MM-YYYY')}.xlsx`
					);
