		...
		Observable.forkJoin(
			...
			.subscribe(
				(res) => {
					...
					this.professionals = [ { ...optAll } ].concat(
						res[0].map((pro) => {
							return {
  ...
  */
   ...
   */
	public downloadReport() {
		...
		this.http
			.get(`${environment.apiUrl}/api/reports/hoursnursing`, {
				params: params,
			...
			.subscribe(
				(res) => {
					FileSaver.saveAs(
						...
						`Reporte Horas de Enfermeria - ${this.filters.ReportType === 1
							? 'Consolidado'
							: 'Detallado'} - ${moment(Date.now()).format('DD-MM-YYYY')}.xlsx`
					);
